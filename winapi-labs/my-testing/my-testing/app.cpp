...
App::App(VOID)
{
	...
	catch (const exception& ex)
	{
		...
		MessageBox(nullptr, wstring(begin(message), end(message)).c_str(), L"ERROR", MB_ICONERROR | MB_OK);
...
int App::run(VOID)
{
	...
	return reinterpret_cast<int>(msg.wParam);
}

LRESULT App::application_proced(HWND hWnd, UINT uMsg, LPARAM lParam, WPARAM wParam)
{
	App* app;
	if (uMsg == WM_NCCREATE)
	{
...
LRESULT App::window_proced(HWND hWnd, UINT uMsg, LPARAM lParam, WPARAM wParam)
{
